...
def predict_from_files2(
                       predictor1,
                       predictor2,
                       list_of_lists_or_source_folder: Union[str, List[List[str]]],
                       output_folder_or_list_of_truncated_output_files: Union[str, None, List[str]],
                       output_folder_or_list_of_truncated_output_files2: Union[str, None, List[str]],
                       save_probabilities: bool = False,
                       overwrite: bool = True,
                       num_processes_preprocessing: int = default_num_processes,
                       num_processes_segmentation_export: int = default_num_processes,
    ...
    if isinstance(output_folder_or_list_of_truncated_output_files, str):
        output_folder = output_folder_or_list_of_truncated_output_files
        output_folder2 = output_folder_or_list_of_truncated_output_files2
    elif isinstance(output_folder_or_list_of_truncated_output_files, list):
        output_folder = os.path.dirname(output_folder_or_list_of_truncated_output_files[0])
        output_folder2 = os.path.dirname(output_folder_or_list_of_truncated_output_files2[0])
    else:
        output_folder = None
    ...
    if output_folder is not None:
        my_init_kwargs = {}
        for k in inspect.signature(predictor1.predict_from_files).parameters.keys():
            my_init_kwargs[k] = locals()[k]
        my_init_kwargs = deepcopy(
            my_init_kwargs)  # let's not unintentionally change anything in-place. Take this as a
        recursive_fix_for_json_export(my_init_kwargs)
        ...
        # we need these two if we want to do things with the predictions like for example apply postprocessing
        save_json(predictor1.dataset_json, join(output_folder, 'dataset.json'), sort_keys=False)
        save_json(predictor1.plans_manager.plans, join(output_folder, 'plans.json'), sort_keys=False)
    #######################

    ########################
    # let's store the input arguments so that its clear what was used to generate the prediction
    if output_folder2 is not None:
        my_init_kwargs = {}
        for k in inspect.signature(predictor2.predict_from_files).parameters.keys():
            my_init_kwargs[k] = locals()[k]
        my_init_kwargs = deepcopy(
            my_init_kwargs)  # let's not unintentionally change anything in-place. Take this as a
        recursive_fix_for_json_export(my_init_kwargs)
        maybe_mkdir_p(output_folder2)
        save_json(my_init_kwargs, join(output_folder2, 'predict_from_raw_data_args.json'))

        # we need these two if we want to do things with the predictions like for example apply postprocessing
        save_json(predictor2.dataset_json, join(output_folder2, 'dataset.json'), sort_keys=False)
        save_json(predictor2.plans_manager.plans, join(output_folder2, 'plans.json'), sort_keys=False)
    #######################

    # check if we need a prediction from the previous stage
    if predictor1.configuration_manager.previous_stage_name is not None \
            and predictor2.configuration_manager.previous_stage_name is not None:
        assert folder_with_segs_from_prev_stage is not None, \
            f'The requested configuration is a cascaded network. It requires the segmentations of the previous ' \
            f'stage1 ({predictor1.configuration_manager.previous_stage_name}) as input. Please provide the folder where' \
            f'stage2 ({predictor2.configuration_manager.previous_stage_name}) as input. Please provide the folder where' \
            f' they are located via folder_with_segs_from_prev_stage'

    # sort out input and output filenames
    list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files = \
        predictor1._manage_input_and_output_lists(list_of_lists_or_source_folder,
                                            output_folder_or_list_of_truncated_output_files,
                                            folder_with_segs_from_prev_stage, overwrite, part_id, num_parts,
                                            save_probabilities)

    list_of_lists_or_source_folder2, output_filename_truncated2, seg_from_prev_stage_files2 = \
        predictor2._manage_input_and_output_lists(list_of_lists_or_source_folder,
                                                  output_folder_or_list_of_truncated_output_files2,
                                                  folder_with_segs_from_prev_stage, overwrite, part_id, num_parts,
                                                  save_probabilities)
    if len(list_of_lists_or_source_folder) == 0:
        return

    data_iterator = predictor1._internal_get_data_iterator_from_lists_of_filenames(list_of_lists_or_source_folder,
                                                                             seg_from_prev_stage_files,
                                                                             output_filename_truncated,
                                                                             num_processes_preprocessing)
    data_iterator2 = predictor2._internal_get_data_iterator_from_lists_of_filenames(list_of_lists_or_source_folder2,
                                                                                   seg_from_prev_stage_files2,
                                                                                   output_filename_truncated2,
                                                                                   num_processes_preprocessing)

    return predictor1.predict_from_data_iterator(data_iterator, save_probabilities, num_processes_segmentation_export),
    predictor2.predict_from_data_iterator(data_iterator2, save_probabilities, num_processes_segmentation_export)
...
if __name__ == '__main__':
    ...
    # predict_from_files2(predictor, predictor2,
    #                     "/public/flare/testImage/",
    #                     '/public/flare/testImage/',
    #                     '/public/flare/testOut/',
    #                     save_probabilities=False, overwrite=False,
    #                     num_processes_preprocessing=2, num_processes_segmentation_export=2,
    #                     folder_with_segs_from_prev_stage=None, num_parts=1, part_id=0)

    predictor.predict_from_files("/public/wangtao/nnUNet_data/nnUNet_raw/Dataset804_Bignet2/imagesTr/",
                                 '/public/flare/testOut/',
                                 save_probabilities=False, overwrite=False,
                                 num_processes_preprocessing=8, num_processes_segmentation_export=8,
                                 folder_with_segs_from_prev_stage=None, num_parts=1, part_id=0)

    predictor2.predict_from_files("/public/wangtao/nnUNet_data/nnUNet_raw/Dataset804_Bignet2/imagesTr/",
                                 '/public/flare/testOut/',
                                 save_probabilities=False, overwrite=False,
    ...
    img, props = SimpleITKIO().read_images(["/public/flare/validation/FLARE23Ts_0001_0000.nii.gz"])
    ...
    ret = predictor.predict_from_data_iterator(my_iterator(predictor, [img], [props]),
